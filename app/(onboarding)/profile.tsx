...
const ACTIVITY_OPTIONS: {
  ...
  label: string;
  description: string;
}[] = [ // Added helper for type inference if needed, but keeping existing structure
    {
      value: "sedentary",
      label: "Sedentario",
    ...
    >
      ...
      >
        <Text className="text-2xl font-bold text-gray-900 mb-6">
          {isAddMember ? "Dati del membro" : "Parliamo di te"}
        </Text>

        {/* Name */}
        <Text className="text-gray-700 font-medium mb-2">
          {isAddMember ? "Come si chiama?" : "Come ti chiami?"}
        </Text>
        <Controller
          control={control}
          name="name"
          render={({ field: { onChange, onBlur, value } }) => (
            <TextInput
              ...
              placeholder={isAddMember ? "Nome del familiare" : "Il tuo nome"}
              onBlur={onBlur}
          ...
          render={({ field: { onChange } }) => (
            <View className="flex-row gap-3 mb-4">
              {SEX_OPTIONS.map((opt) => (
                <Pressable
                  key={opt.value}
                  className={`flex-1 py-3 rounded-xl border ${selectedSex === opt.value
          ...
          render={({ field: { onChange } }) => (
            <View className="gap-2">
              {ACTIVITY_OPTIONS.map((opt) => (
                <Pressable
                  key={opt.value}
                  className={`py-3 px-4 rounded-xl border ${selectedActivity === opt.value
                    ? "bg-primary-100 border-primary-500"
                    : "bg-gray-50 border-gray-200"
                    }`}
                  onPress={() => onChange(opt.value)}
