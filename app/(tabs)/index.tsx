...
export default function HomeScreen() {
  ...
  const { data: mealPlan } = useMealPlan(primaryMemberId, weekStart);
  ...
  return (
    <View className="flex-1 bg-ui-50">
      ...
      >
        <View className="flex-row justify-between items-start mb-4">
          <View>
            ...
            <Text className="text-3xl font-bold text-ui-900 mt-1">{userName.split(" ")[0]}</Text>
          </View>
        ...
        <View className="flex-row gap-4 mt-2">
          <View className="flex-1 bg-ui-50 p-3 rounded-2xl flex-row items-center gap-3">
            ...
            <View>
              ...
              <Text className="text-lg font-bold text-ui-900">{completedKcal}</Text>
            </View>
          </View>
          <View className="flex-1 bg-ui-50 p-3 rounded-2xl flex-row items-center gap-3">
            <View className="w-10 h-10 rounded-full bg-white items-center justify-center">
      ...
      >
        {/* Next Meal Highlight */}
        <Text className="text-lg font-bold text-ui-900 mb-4 px-1">Prossimo Pasto</Text>

        {nextMeal ? (
          <MealCard
            meal={nextMeal}
            onPress={() => router.push(`/recipe/${nextMeal.recipeId}`)}
            onSwap={() => router.push({ pathname: "/meal-swap", params: { mealId: nextMeal.id } })}
          />
        ) : (
          <Card className="items-center py-8 border-dashed border-2 border-ui-200 bg-transparent shadow-none" variant="flat">
            <Text className="text-4xl mb-2">🎉</Text>
            <Text className="text-ui-500 font-medium">Tutto fatto per oggi!</Text>
          </Card>
        )}

        {/* Weekly Overview */}
        <View className="flex-row justify-between items-end mt-8 mb-4 px-1">
          <Text className="text-lg font-bold text-ui-900">Riepilogo Settimana</Text>
        ...
        <Card className="flex-row items-center gap-6">
          ...
          <View className="flex-1 gap-4">
            <View>
              <Text className="text-xs font-bold text-ui-400 uppercase tracking-widest mb-1">Target Giornaliero</Text>
              <ProgressBar current={completedKcal} target={dailyTarget} />
            </View>
            <View>
              <Text className="text-xs font-bold text-ui-400 uppercase tracking-widest mb-1">Progresso Week</Text>
              <ProgressBar current={weeklyActual} target={weeklyTarget} color="#22c55e" />
            </View>
          </View>
        </Card>

        {/* Inspiration / Blog */}
        <Text className="text-lg font-bold text-ui-900 mt-8 mb-4 px-1">Ispirazione</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-5 px-5" contentContainerStyle={{ gap: 16, paddingRight: 20 }}>
          <Pressable className="w-64 h-36 bg-brand-500 rounded-3xl p-5 justify-between shadow-lg shadow-brand-500/30">
            ...
            <Text className="text-white text-xl font-bold">5 trucchi per la meal prep perfetta</Text>
          </Pressable>
          <Pressable className="w-64 h-36 bg-ui-800 rounded-3xl p-5 justify-between shadow-lg shadow-ui-900/20">
            ...
            <Text className="text-white text-xl font-bold">Le ricette più votate di Dicembre</Text>
          </Pressable>
        </ScrollView>
