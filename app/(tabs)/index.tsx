...
export default function HomeScreen() {
	...
	/* Fetch family members to get the name if user_metadata is empty */
	const { data: members } = useFamilyMembers();
	const primaryMember = members?.find((m) => m.isPrimary);

	// Try auth metadata first, then fall back to primary member name, then "Chef"
	const userName = user?.user_metadata?.name || primaryMember?.name || "Chef";
