...
export default function RecipesScreen() {
  ...
  // Filter with debounced query for better performance
  const filteredRecipes = debouncedQuery.trim()
    ? recipes?.filter(
      (r) =>
        r.nameIt.toLowerCase().includes(debouncedQuery.toLowerCase()) ||
        r.nameEn.toLowerCase().includes(debouncedQuery.toLowerCase()),
    )
    : recipes;
  ...
  return (
    <View className="flex-1 bg-ui-50" style={{ paddingTop: insets.top }}>
      ...
      {/* Recipe Grid - FlashList for better performance */}
