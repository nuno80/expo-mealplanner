...
export default function PlanScreen() {
	...
	const toggleMealComplete = useToggleMealComplete();

	const handleDayOptions = useCallback(
		(day: number, hasSnacks: boolean) => {
			if (!mealPlan?.id) return;

			Alert.alert("Opzioni Giorno", "Gestisci i pasti per questo giorno", [
				{
							...
							return (
								<View key={day}>
									...
									<View className="flex-row items-end gap-3 mb-3 ml-1">
										...
										>
											<Text className="text-ui-400 text-lg font-bold tracking-widest pl-2">
												•••
											</Text>
										</Pressable>
									</View>
									...
									<View className="gap-0">
										{meals.map((meal) => (
											<MealCard
												...
												onComplete={() => toggleMealComplete.mutate(meal.id)}
											/>
										))}
										{meals.length === 0 && (
