            ...
            {weightChange && (
              <View className={`${badgeBg} px-2 py-1 rounded-lg mb-2`}>
                <Text className={`${badgeText} font-bold text-xs`}>
                  {isGain ? "+" : ""}{weightChange.change} kg (30gg)
                </Text>
              </View>
            )}
          </View>
          ...
          <View className="bg-white rounded-2xl overflow-hidden shadow-sm">
            {history.length === 0 ? (
              ...
              history.map((log, index) => (
                <View
                  key={log.id}
                  className={`p-4 flex-row justify-between items-center ${index < history.length - 1 ? "border-b border-gray-100" : ""
                    }`}
                >
                  ...
                  <Text className="font-bold text-gray-900">{log.weightKg} kg</Text>
                </View>
              ))
            )}
          </View>
