...
export default function AddMemberScreen() {
  ...
  useEffect(() => {
    const { sex, weightKg, heightCm, birthYear, activityLevel, goal } = watchedValues;
    ...
    if (sex && weightKg && heightCm && birthYear && activityLevel) {
      ...
      const defaultAdj = DEFAULT_CALORIE_ADJUSTMENTS[goal as keyof typeof DEFAULT_CALORIE_ADJUSTMENTS] || 0;
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: FAMILY_KEYS.list(user?.id ?? "") });
      router.back();
    },
    onError: (error) => {
      Alert.alert("Errore", "Impossibile aggiungere il membro: " + error.message);
    },
  });
    ...
    >
      <View className="flex-row justify-between items-center px-4 py-3 border-b border-gray-100">
        <Pressable onPress={() => router.back()} className="p-2">
          <Text className="text-primary-600 font-semibold text-lg">Annulla</Text>
        </Pressable>
                ...
                <View className="space-y-2">
                  {[
                    ...
                    <Pressable
                      ...
                      className={`p-3 rounded-xl border ${value === opt.val
                          ? "bg-primary-50 border-primary-500"
                          : "bg-white border-gray-200"
                        }`}
                    >
                      <Text
                        className={`font-medium ${value === opt.val ? "text-primary-700" : "text-gray-600"
                          }`}
                      >
                        {opt.label}
                      </Text>
                ...
                <View className="flex-row space-x-2">
                  {[
                    ...
                    <Pressable
                      key={opt.val}
                      onPress={() => onChange(opt.val)}
                      className={`flex-1 p-3 rounded-xl border items-center ${value === opt.val
                          ? "bg-primary-50 border-primary-500"
                          : "bg-white border-gray-200"
                        }`}
                    >
                      <Text
                        className={`font-semibold ${value === opt.val ? "text-primary-700" : "text-gray-600"
                          }`}
                      >
                        {opt.label}
                      </Text>
