...
export default function ShoppingListModal() {
	...
	const weekOffset = params.weekOffset
		? Number.parseInt(params.weekOffset, 10)
		: 0;
	...
	const { data: realItems = [], isLoading } = useShoppingList(
		selectedMemberId,
		weekStart,
		daysToFetch,
	);
	...
	const items =
		realItems.length > 0
			...
			: MOCK_ITEMS.filter(
					(_, i) =>
						periodOption === "week" || selectedDays.includes((i % 7) + 1),
				);
						...
						{(["week", "days"] as PeriodOption[]).map((option) => (
							<Pressable
								...
								style={{
									...
									alignItems: "center",
									backgroundColor:
										periodOption === option ? "#f97316" : "#ffffff",
									borderWidth: 1,
							...
							>
								<Text
									style={{
										fontWeight: "600",
										color: periodOption === option ? "#ffffff" : "#374151",
									}}
								>
			...
			{isLoading ? (
				...
				>
					...
					<Text style={{ color: "#6b7280", textAlign: "center" }}>
						{selectedMemberId
							? "Genera un piano settimanale per vedere la lista della spesa."
							: "Seleziona un membro della famiglia."}
					</Text>
				</View>
			) : (
				<ScrollView
