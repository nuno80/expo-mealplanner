// ============================================================================
// MEAL PLAN LOGIC (Pure Functions) - v2.4 (Legume-Grain Complementation)
// Adapted from /src/services/mealPlan.logic.ts - Keep in sync with mobile app
// ============================================================================

import type { MealType, SnackPreference } from "@/types";
...
export interface LogicRecipe {
  ...
  carbsPer100g: number;
  fatPer100g: number;
  /** Protein source for complementation logic (Mod 16) */
  proteinSource?: string;
  /** Nutritional tags for Harvard Plate (Mod 18) */
  tags?: string[];
}
...
export function calculateMealComposition(
  ...
  if (isMainMeal && sideRecipes.length > 0) {
    ...
    if (shouldTrigger) {
      ...
      if (gapKcal > 50) {
        // Filter to HIGH-DENSITY sides only (≥150 kcal/100g)
        ...
        if (isLegumeDish) {
          debugLog.push(`[GapFill] 🫘 Legume Main detected (${mainRecipe.proteinSource}), forcing GRAIN sides for protein complementation`);
          ...
          const grainSides = sideRecipes.filter(
            s => s.carbsPer100g > 25
          );

          if (grainSides.length > 0) {
            // Prefer dense grains (bread ~250kcal/100g) over light ones
            const denseGrains = grainSides.filter(s => s.kcalPer100g >= MIN_SIDE_DENSITY);
            if (denseGrains.length > 0) {
              highDensitySides = denseGrains;
              debugLog.push(`[GapFill] ✅ Found ${denseGrains.length} grain+dense sides (CHO >25g AND ≥${MIN_SIDE_DENSITY} kcal/100g) for legume complementation`);
            } else {
              highDensitySides = grainSides;
              debugLog.push(`[GapFill] ⚠️ Using ${grainSides.length} grain sides (CHO >25g/100g) - lower density but necessary for complementation`);
            }
          } else {
            debugLog.push(`[GapFill] ⚠️ No grain sides found for legume complementation, falling back to starchy`);
        ...
        // Mod 12 (Carb Safeguard): If main is low-carb (<12g/100g) and NOT legumes, prefer starchy sides
          ...
          debugLog.push(`[GapFill] 🍞 Low Carb Main detected (<12g/100g), forcing starchy sides`);
          ...
          if (starchyAndDense.length > 0) {
            ...
            debugLog.push(`[GapFill] ✅ Found ${starchyAndDense.length} starchy+dense sides (CHO >20 AND ≥${MIN_SIDE_DENSITY} kcal/100g)`);
          } else {
            // Fallback: Just starchy (including potatoes 90kcal/100g) - but warn
            const starchySides = sideRecipes.filter(s => s.carbsPer100g > 20);
            if (starchySides.length > 0) {
              highDensitySides = starchySides;
              debugLog.push(`[GapFill] ⚠️ No starchy+dense sides, falling back to ${starchySides.length} starchy sides (CHO >20g/100g)`);
            } else {
              debugLog.push(`[GapFill] ⚠️ No starchy sides found, keeping density-only filter`);
            }
          }
        }
        ...
        if (highDensitySides.length > 0) {
          ...
          // DOUBLE SIDE: If there's remaining gap after cap, add BREAD only
          // (Mod 17: Avoid redundant grains like rice + rice cakes)
...
export function validateSideDishFallback(
  ...
  const result = calculateMealComposition(targetKcal, 2000, mainRecipe, [], true);
