	...
	return (
		...
		>
			<View className="items-center">
				...
				<View
					className={`mt-2 px-2 py-0.5 rounded-full ${isOver ? "bg-red-200" : "bg-brand-200"}`}
				>
					<Text
						className={`text-[10px] font-bold ${isOver ? "text-red-700" : "text-brand-700"}`}
					>
						{percentage}%
					</Text>
				</View>
		...
		<View className="w-full">
			...
			{showLabel && (
				<View className="flex-row justify-between mt-1.5">
					<Text
						className={`text-xs font-medium ${isOver ? "text-red-500" : "text-ui-600"}`}
					>
