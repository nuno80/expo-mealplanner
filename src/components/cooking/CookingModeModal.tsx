...
export function CookingModeModal({
	...
	const cookedPortions = useMemo(() => {
		return activeMembersData.map((d) => {
			const target = calculateMealTarget(d.member, d.mealType);
			const grams = calculateCookedPortion(recipe, target);
			return {
				member: d.member,
				targetKcal: target,
				grams,
	...
	const totalCookedExpected = cookedPortions.reduce(
		(sum, p) => sum + p.grams,
		0,
	);
				...
				>
					...
					<View className="mb-8">
						...
						{cookedPortions.length === 0 ? (
							...
							cookedPortions.map((p) => (
								<View
									key={p.member.id}
									className="bg-white rounded-xl p-4 mb-3 border border-gray-100 shadow-sm flex-row justify-between items-center"
								>
