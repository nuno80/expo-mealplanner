...
/**
 * Toggle meal completion status (mark as completed/uncompleted).
 */
export function useToggleMealComplete() {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (plannedMealId: string) => toggleMealComplete(plannedMealId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: mealPlanKeys.all });
		},
	});
...
/**
 * Get weekly progress (kcal/macros from completed meals).
 * Used for progress charts and tracking.
 */
export function useWeeklyProgress(
	...
	return useQuery({
		...
		queryFn: () =>
			familyMemberId && weekStart
				? getWeeklyProgress(familyMemberId, weekStart)
				: Promise.resolve(null),
		enabled: !!familyMemberId && !!weekStart,
