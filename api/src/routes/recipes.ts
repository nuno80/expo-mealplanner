...
recipes.get("/", async (c) => {
  ...
  try {
    ...
    // Ingredients and steps are independent. Running both Turso requests at the
    // same time removes one full network round trip from this hot endpoint.
    ...
    // Group ingredients and steps by recipe_id
...
recipes.get("/:id", async (c) => {
  ...
  try {
    ...
    // Both child collections depend only on the recipe ID, so overlap their
    // remote database calls instead of paying their latency sequentially.
    const [ingredientsResult, stepsResult] = await Promise.all([
      db.execute({
        sql: `
          SELECT
